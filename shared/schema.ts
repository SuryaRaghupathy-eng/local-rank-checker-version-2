...
export const searches = pgTable("searches", {
  ...
  deviceType: varchar("device_type", { length: 10 }).notNull().default('desktop'),
  geoGridEnabled: boolean("geo_grid_enabled").notNull().default(false),
  geoGridConfig: jsonb("geo_grid_config"),
  ...
  totalLocalPackMatches: integer("total_local_pack_matches").notNull().default(0),
...
export const rankingResults = pgTable("ranking_results", {
  ...
  isLocalPack: boolean("is_local_pack").notNull().default(false),
  ...
  deviceTypeIdx: index("ranking_results_device_type_idx").on(table.deviceType),
  localPackIdx: index("ranking_results_local_pack_idx").on(table.isLocalPack),
}));
